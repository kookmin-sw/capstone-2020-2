...
const renderActiveShape = (props) => {
    ...
    return (
      <g>
        <text font-size="30px" x={cx} y={cy} dy={8} textAnchor="middle" fill={fill}>{payload.emotionTag}</text>
        <Sector
          cx={cx}
          cy={cy}
        ...
        <text  x={ex + (cos >= 0 ? 1 : -1) * 12} y={ey} textAnchor={textAnchor} fill="#333">{`${multi}`} </text>
    ...
    return (
      <div class="full-container">
        ...
        <Pie
          ...
          label
        >{
            this.state.signalData.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)
          }
        </Pie>
      </PieChart>
